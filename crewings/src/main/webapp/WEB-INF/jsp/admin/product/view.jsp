        ...
        <div id="adm_sc_con">
            <div class="adm_sc_size">
                ...
                <section class="adm_sc_txt">
                	<div>
                    <form action="./update.do" method="post" name="insertForm" id="insertForm" enctype="multipart/form-data">
                        <input type="hidden"  name="csrf" value="${CSRF_TOKEN}" />
                        <input type="hidden" id="image_change_bool" name="image_change_bool" value="false">
                        <input type="hidden" id="location" name="location" value="${model.view.location }">
                                ...
                                <div class="member_input_wrap">
                                    <ul class="member_input">
                                        <li>
                                            ...
                                            <input class="input_size mr" type="text" id="" name="name" value="${model.view.name }" >
                                        </li>
                                        <li>
                                        	<span class="list_t">상품 가격</span>
                                        	<input class="input_title" type="text" name="price"  value="${model.view.price }" >
                                        </li>
                                        <li>
                                        	<span class="list_t">상품 카테고리</span>
                                        	<select name="category" class="input_title" >
                                        		<option value="">카테고리를 선택해주세요</option>
                                        		<option value="1" <c:if test="${item.category == '1' }">checked="checked"</c:if>>상품권.생활.도서</option>
                                        		<option value="2" <c:if test="${item.category == '2' }">checked="checked"</c:if>>커피.음료</option>
                                        		<option value="3" <c:if test="${item.category == '3' }">checked="checked"</c:if>>베이커리.도넛.떡</option>
                                        		<option value="4" <c:if test="${item.category == '4'}">checked="checked"</c:if>>아이스크림.빙수</option>
                                        		<option value="5" <c:if test="${item.category == '5' }">checked="checked"</c:if>>치킨.버거.피자</option>
                                        		<option value="6" <c:if test="${item.category == '6' }">checked="checked"</c:if>>편의점.마트</option>
                                        	</select>
                                        </li>
                                        <li>
                                        ...
                                        <li>
                                        	...
                                        	<input class="input_title" type="text" name="coment"  value="${model.view.coment }">
                                        </li>
                                    </ul>
                                </div>
                            </div>
...
$(document).ready(function(){
	...
	var location_val = '${model.view.location}';
	var location_list = [];
	location_list = location_val.split('%23');
	
	for(i=1; i < location_list.length; i ++){
		
		var html = `<span class="location_list_div">`+location_list[i]+`</span>`; 
